...
데일리 노트를 쓴 뒤 채팅창에 **`데일리 정리`** 라고 입력하면, 그 노트를 읽어 레퍼런스 문서에 자동 반영합니다.

| 입력 | 동작 |
|---|---|
| `데일리 정리` | 최근 데일리 노트 (확인 후 실행) |
| `데일리 정리 (8.8.)` | 해당 날짜 노트 |
| `데일리 정리 전체` | 미반영분 전부 |

**하는 일**: 회사 블록 파싱 → 내 딜/학습 딜 판정 → 딜 노트 생성·병합 → 데일리에 위키링크 주입 → 신규 개체 제안

**안전장치**: 실행 전 git 체크포인트 · 기존 노트는 병합만(덮어쓰기 없음) · 새 허브 노트는 승인 후 생성 · 같은 노트를 두 번 돌려도 안전

**판정 정확도를 높이려면** 데일리에 이렇게 표기하세요:
```markdown
**한빛리테일** [내딜] | 커머스, 생활용품/가전     ← 내 딜로 강제
```
콜 로그 표에 있는 회사는 자동으로 내 딜 처리됩니다.

> 스킬 정의: `.claude/skills/daily-ref/` (볼트와 함께 git 백업됨)
